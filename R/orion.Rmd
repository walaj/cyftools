...
```{r kras-km}
g <- km_plot(rrt, parm="KRAS", parm.name="KRAS", flip=FALSE)
save_pdf(g, "kras_km.pdf", width=4,height=3)
```
...
if (FALSE) {
  ...
  lapply(seq_along(nn)[11], function(n) {
    ...
    dtt <- mclapply(files, function(f) {
      ...
      if (system(paste("cysift count -A 8",f, nn[n]), intern=TRUE)=="0") {
        cat("no manual annotations for ", basename(f),"\n")
        return(data.table())
      }
      ...
      dt[,c("sid","cid","cflag","pflag","x","y","Hoechst","AF1","CD31","CD45","CD68","Argo550","CD4","FOXP3","CD8a","CD45RO",
            "CD20","PD-L1","CD3e","CD163","E-cadherin","PD-1","Ki67","Pan-CK","SMA","Area","Region") := NULL]
      dt[ , allcount       := fread(cmd=paste("cysift count", tmpfile))$V1]
      
      # individual cell counts
      drr = fread(cmd=paste("cysift cellcount", tmpfile," - -a 133120 -a 10240 -a 36864 -a 32832 -a 0 -a 135168 -a 131328 | cysift view - -R"))
      drr[,c("sid","cid","cflag","pflag","x","y","Hoechst","AF1","CD31","CD45","CD68","Argo550","CD4","FOXP3","CD8a","CD45RO",
             "CD20","PD-L1","CD3e","CD163","E-cadherin","PD-1","Ki67","Pan-CK","SMA") := NULL]
  ...
  # load the saved data
  dtr <- rbindlist(lapply(paste0("~/Sorger/cache/orion_",names(nn),".rds"), readRDS))
  rr <- merge(data.table::copy(redcap), dtr, by = "Slide_ID", all.x = TRUE)
  rrt <- data.table::copy(rr[margin=="tumor"])
}
```

```{r km-by-t-cell-count}
...
hard_coded_tip_combo_50l_manual <- c("LSP14468","LSP14483","LSP14418","LSP14388","LSP14438","LSP14458","LSP14448","LSP14393") 

hard_coded_tip_cd3_50r <-     c("LSP14468","LSP14448","LSP14403","LSP14483","LSP14458","LSP14418","LSP14388","LSP14423","LSP10388","LSP10771")
rr[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_cd3_50r, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
...
hard_coded_tip_cd8_cd3_50r <- c("LSP14468","LSP14448","LSP14403","LSP14483","LSP14458","LSP14418","LSP14388","LSP14423","LSP10388","LSP10771")
rr[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_cd8_cd3_50r, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
g <- km_plot(rr[pMMR=="pMMR" & !duplicated(Slide_ID)], parm="tipMMR", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE)
hard_coded_tip_cd8_50r <-     c("LSP14483","LSP14468","LSP14418","LSP14458","LSP10771","LSP10388","LSP14403","LSP10452","LSP10727","LSP14493")
rr[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_cd8_50r, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
g <- km_plot(rr[pMMR=="pMMR" & !duplicated(Slide_ID)], parm="tipMMR", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE)
rr[, tipMMR := factor(ifelse(Slide_ID %in% c(hard_coded_tip_cd8_50r,hard_coded_tip_cd3_50r,hard_coded_tip_cd8_cd3_50r), ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
g <- km_plot(rr[pMMR=="pMMR" & !duplicated(Slide_ID)], parm="tipMMR", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE, pfs=TRUE)
...
hard_coded_tip_old <- c("LSP14418", "LSP14398", "LSP14423", "LSP14438", "LSP14393", "LSP14403", "LSP14458", "LSP10771", "LSP14388", "LSP14468", "LSP14483", "LSP10388", "LSP14448")

rr[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_combo_50l_manual, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
rrt <- rr[margin=="tumor"]
```
...
rrt[, ratio := panckcd3count / allcount]
...
rr[, pdl1cd163_ratio := pdl1cd163count / allcount]
rr[, pdl1panckcount_ratio := pdl1panckcount / allcount]
rr[, m0_tumor_ratio := pdl1cd163count / pdl1panckcount]

g = # Assuming your data.table is named rr
    ggplot(rr, aes(x = margin, y = m0_tumor_ratio, fill = tipMMR)) +
    geom_boxplot(alpha = 0.7, outlier.shape=NA) +                 # Draw the boxplot with some transparency
    #geom_jitter(width = 0.2, size = 1.5, color="black") +  # Add jittered points
    facet_wrap(~margin, scales="free", nrow=1) +      # Create facets for each margin type
  scale_fill_manual(values = pMMR_colors) +    theme_minimal() + 
  #scale_y_log10(labels = scales::label_number()) +
    labs(x = "Margin Type", 
         y = "m0 / tumor PD-L1 ratio", 
         fill = "MMR Status", 
         color = "MMR Status") +                # Labeling axes and legend
  theme_jw()

rrt <- data.table::copy(rr[margin=="stroma_margin"])
...
known <- rbind(known, data.table(variable=c("PNI","LVI","mucinous","p53","KRAS","BRAF","Gender"),hr=c(2.05,2.13,1.63,0.92,1.26,1.57,1.1),lower=c(1.38,1.42,0.93,0.61,0.84,0.64,0.73),upper=c(3.06,3.21,2.88,1.4,1.90,3.87,1.65),pmid="30894685",type="dfs",note="DFS"))
known[, source := "lit"]
...
g <- ggplot(combined_data, aes(y = variable, x = hr, color = source)) +
  ...
  scale_color_manual(v galues = c("us" = "black", "lit" = "red")) +
...
save_pdf(g, "forest_lit_compare.pdf", width=4,height=3)
```
...
cox_model <- coxph(survival_object ~ redcap$Gender[ix] + redcap$mucinous[ix] + redcap$Grade[ix] + redcap$LVI[ix] + redcap$PNI[ix] + redcap$stage_num[ix] + redcap$APC[ix] + redcap$KRAS[ix] + redcap$p53[ix] +
...
# Initialize results
covariates <- c("Gender", "Age", "stage_num","mucinous", "Grade", "LVI", "PNI", "TIL_num","tmb","APC", "KRAS", "p53", "BRAF", "PIK3CA", "tipMMR")
