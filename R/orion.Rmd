...
redcap[, TIL_num := c("absent"=0,"mild"=1,"moderate"=2,"marked"=3)[TIL]]
...
redcap[, PFSC := ifelse(redcap$PFSCensor==0,1,0)]
```
...
## MUTATION COUNT PANEL B
# reshape the mutation data
long_data <- redcap %>%
  gather(key = "Mutation", value = "Status", APC, TP53, KRAS, BRAF, PIK3CA, SOX9, FBXW7, TCF7L2, CTNNB1, PTEN, NRAS) %>%
...
g_mut <- ggplot(long_data, aes(x = Mutation, fill = as.factor(pMMR))) +
...
## KRAS KM PLOT C
g_kras <- km_plot(redcap, parm="KRAS", parm.name="KRAS", flip=FALSE) + 
  theme(legend.position = c(0, 0), # Bottom left corner
        legend.justification = c(-0.2, -0.3), # Coordinates relative to the plot area
        legend.box.margin = margin(6, 6, 6, 6), # Adjust spacing around the legend box if necessary
        legend.margin = margin(-10, -10, -10, -10),
        text = element_text(family = "Helvetica"),
  ...
  scale_color_manual(name="",values=c("0"="#4575b4", "1"="#d73027"),labels=c("0"="KRAS-wt","1"="KRAS-mut"))+
  scale_fill_manual(values=c("0"="#4575b4", "1"="#d73027"), guide="none") + 
  #guides(fill = guide_legend(byrow = TRUE)) + 
  coord_fixed(ratio=6)
#save_pdf(g, "kras_km.pdf", width=3.5,height=3)

## MUTATION FOREST PLOT 
# Known HRs by effect
known <- data.table()
...
known <- rbind(known, data.table(variable=c("PNI","LVI","mucinous","TP53","KRAS","BRAF","Gender"),hr=c(2.05,2.13,1.63,0.92,1.26,1.57,1.1),lower=c(1.38,1.42,0.93,0.61,0.84,0.64,0.73),upper=c(3.06,3.21,2.88,1.4,1.90,3.87,1.65),pmid="30894685",type="dfs",note="DFS"))
known[, source := "lit"]

# HR by mutation
hr <- rbindlist(lapply(nn <- c("APC","TP53","KRAS","BRAF","PIK3CA", "LVI","PNI","Grade", "mucinous", "Gender"), function(x) {
 ...
 dt <- km_plot(redcap, x, x, c("0"="WT", "1"="Mutated"),return.model=TRUE, flip=flip)
...
cdd <- combined_data[!variable %in% c("PNI","LVI") & !(variable == "Gender" & type=="OS")]
replacements <- c("Mucinous" = "mucinous", "Sex (Male)" = "Gender")
# Loop over the replacements and update the 'variable' column
for (old in names(replacements)) {
  cdd[variable == replacements[old], variable := old]
  hr[variable == replacements[old], variable := old]
}
cdd$variable <- factor(cdd$variable, levels = hr$variable[order(-hr$hr)])
g_forest <- ggplot(cdd, aes(y = variable, x = hr, color = source)) +
  geom_point(aes(x = hr), size = 1.5, position = position_dodge(width = 0.3)) +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2, position = position_dodge(width = 0.3)) +
  scale_color_manual(values = c("us" = "black", "lit" = "red"), labels=c("This cohort","Literature")) +
  labs(x = "Hazard ratio") +
  theme_jw() +
  geom_vline(aes(xintercept = 1), color = "black", linetype = "dashed") +  # Add vertical red line at HR = 1
   theme(legend.position = c(0.50, 0.7), # Bottom left corner
        legend.justification = c(-0.2, -0.3), # Coordinates relative to the plot area
        legend.box.margin = margin(6, 6, 6, 6), # Adjust spacing around the legend box if necessary
        legend.margin = margin(-10, -10, -10, -10),
        text = element_text(family = "Helvetica"),
    ...
    scale_x_continuous(limits=c(0,4.7),breaks=c(0,1,2,3,4), expand=c(0,0), labels=c(0,1,2,3,4))

## ASSEMBLE THE FIGURE
combined_plot_row1 <- ggarrange(blank_plot, g_mut, g_kras, ncol = 3, nrow = 1, widths = c(3.5, 3.5, 3.5), labels=c("A","B","C"))
combined_plot_row2 <- ggarrange(g_forest, blank_plot, ncol=2, nrow=1, widths=c(3.5, 7), labels=c("D"))
combined_plot <- ggarrange(combined_plot_row1, combined_plot_row2, nrow=2, widths=c(10.5, 10.5))
save_pdf(combined_plot, "Figure1_R.pdf", width=10.5, height=6)
```
...
lapply(seq_along(nn), function(n) {
  ...
  dtt <- mclapply(files, function(f) {
    ...
    if (system(paste("cysift count",f, nn[n]), intern=TRUE)=="0") {
    ...
    if (grepl("scramble", names(nn)[n]) && grepl("lock",names(nn)[n])) {
      ccmd=paste("cysift scramble -P ",f, "- | cysift filter -T -", nn[n], tmpfile)
    # scramble, without lock
    } else if (grepl("scramble", names(nn)[n])) {
      ccmd=paste("cysift scramble", f,"- | cysift filter -T -", nn[n], tmpfile)
    # otherwise do the margin filter
    } else { 
      ccmd=paste("cysift filter -T",f, nn[n],tmpfile)
    }
    system(ccmd)
    
    # read the means for that margin
    dt <- fread(cmd=paste("cysift mean", tmpfile, " - | cysift view - -R"))
    ...
    drr = fread(cmd=paste("cysift cellcount", tmpfile," - -a 133120 -a 10240 -a 36864 -a 32832 -a 0 -a 135168 -a 131328 -a 4224 -a 131200 -a 192 -a 384 | cysift view - -R"))
  ...
  saveRDS(dtr, paste0("~/Sorger/cache/orion_",names(nn)[n], ".rds"), compress=FALSE)
})

# load the saved data
dtr <- rbindlist(lapply(paste0("~/Sorger/cache/orion_",names(nn),".rds"), readRDS), fill=TRUE)
rr <- merge(data.table::copy(redcap), dtr, by = "Slide_ID", all.x = TRUE)
#rr <- rr[Slide_ID != "LSP14483"]
```

```{r whole-slide-cell-distribution, include=FALSE}
rrt <- data.table::copy(rr[margin=="all"])
setnames(rrt, "E-cadherin_count", "ecadcount")

long_data <- rrt %>%
  select(Slide_ID, cd31count, cd45count, cd68count, cd4count, foxp3count, cd8count, cd45rocount, cd20count, pdl1count, cd3count, cd163count, ecadcount, pd1count, panckcount,smacount) %>%
  gather(key = "Cell_Type", value = "Count", -Slide_ID) %>%
  mutate(Cell_Type = gsub("count", "", Cell_Type))
...
long_data <- merge(rrt[,.(Slide_ID, pMMR)], long_data, by="Slide_ID", all.x=TRUE)

ordered_slide_ids <- long_data %>%
  arrange(desc(pMMR), Total_Count) %>%
  .$Slide_ID %>%
  unique()
...
cell_order <- c("panck", "ecad",   # Group A: Greens
                "sma", "cd31", # Group B: Blues
                "cd163", "cd68", "cd45","cd45ro", # Group C: Reds
                "cd20", "cd3","cd4","cd8", # Group D: Oranges
                "foxp3","pd1","pdl1")  # Group E: Reds
...
colors <- c(rev(brewer.pal(3, "Reds")[1:2]), 
            brewer.pal(3, "Blues")[1:2], 
            brewer.pal(4, "Oranges")[1:4],
            brewer.pal(4, "Greens")[1:4],
            brewer.pal(3, "Purples")[1:3])

# Mapping of cell types to colors
color_map <- setNames(colors, cell_order)

# Create the plot
g <- ggplot(long_data, aes(x = Slide_ID, y = Count/1000, fill = Cell_Type)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = color_map) +
  coord_flip() +
  labs(title = "Cell Counts by Slide ID and Cell Type", x = "Slide ID", y = "Count") +
  theme_jw() + 
  theme(axis.line = element_line(), axis.ticks = element_line()) + 
  scale_y_continuous(expand=c(0,0))
save_pdf(g, "cell_counts_color_bar.pdf", width=7, height=6)


```

```{r CD8-distribution-tip-definition, include=FALSE}
# what margin are we operating on for this section?
rrt <- data.table::copy(rr[margin=="tumor_manual"])
...
g <- ggplot(rrt, aes(x=Slide_ID, y=ratio, fill=pMMR)) + 
  geom_bar(stat="identity", color="black") + ylab("Cells / mm2") + xlab("Sample ID") +
  coord_flip() +  scale_fill_manual(values=pMMR_colors) + theme_jw() + 
  theme(axis.line = element_line(), axis.ticks = element_line()) + 
  scale_x_continuous(expand=c(0,0), breaks=c(0,10,20,30,40,50,60,70)) + scale_y_continuous(expand=c(0,0))
save_pdf(g, "cd8_waterfall.pdf", width=5, height=4)

# from the above, same the HARD CODED TIPMMR samples
hard_coded_tip_liberal <- c("LSP14418","LSP14468","LSP10771","LSP14458","LSP14388","LSP10388","LSP14438","LSP14453","LSP14498","LSP14393","LSP14398","LSP14493","LSP10452","LSP14383","LSP14363") 

# same as above, but only take top 10
hard_coded_tip_strict <- c("LSP14418","LSP14468","LSP10771","LSP14458","LSP14388","LSP10388","LSP14438","LSP14453","LSP14498","LSP14393") 

# define the TIP
rr[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_liberal, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]

# update rrt now that we modified it
rrt[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_liberal, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]

# plot the waterfall again, but now with tipMMR labelled as well
g <- ggplot(rrt, aes(x=samplecount, y=ratio, fill=tipMMR)) + 
  geom_bar(stat="identity", color="black", linewidth=0.3) + ylab("Cells / mm2") + xlab("Sample ID") +
  coord_flip() +  scale_fill_manual(values=pMMR_colors) + theme_jw() + 
  theme(axis.line = element_line(), axis.ticks = element_line()) + 
  scale_x_continuous(expand=c(0,0), breaks=c(0,10,20,30,40,50,60,70)) + scale_y_continuous(expand=c(0,0))
save_pdf(g, "cd8_waterfall_withtip.pdf", width=4, height=3)

## KM plot of tip (liberal = default)
g <- km_plot(rr[pMMR=="pMMR" & !duplicated(Slide_ID)], parm="tipMMR", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE)
save_pdf(g, "tipMMR_km.pdf", width=4,height=3)

## KM plot of tip (strict = exploratory only)
rrp <- data.table::copy(rrt)
rrp[, tipMMR := factor(ifelse(Slide_ID %in% hard_coded_tip_strict, ifelse(pMMR=="dMMR","dMMR","tipMMR"), as.character(pMMR)))]
g <- km_plot(rrp[pMMR=="pMMR" & !duplicated(Slide_ID)], parm="tipMMR", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE)

# which samples are in which of the three groups
# dtu <- sapply(unique(c(hard_coded_tip_cd3_50r,hard_coded_tip_cd8_cd3_50r,hard_coded_tip_cd8_50r)), function(s) {
#   data.table(cd3=s %in% hard_coded_tip_cd3_50r, 
#              cd8=s %in% hard_coded_tip_cd8_50r,
#              combo=s %in% hard_coded_tip_cd8_cd3_50r)
# })
```

```{r tip-beeswarm}
rrt = rr[margin=="tumor_manual"]
tcol <- c("CD4","CD3","CD8","PD1","PD_L1","CD3pPD1","CD45","FOXP3","CD31","CD45RO","SMA","CD163pPDL1","CD68pPDL1","CD20")
rrp <- rrt[, .SD, .SDcols = c(paste0(tcol,"_50r"), "tipMMR")]
rrm <- data.table::melt(rrp, id.vars = c("tipMMR"))
g <- create_beeswarm_plot(rrm, x="tipMMR", y="value") +
  facet_wrap(rrm$variable, scales="free") + 
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme(
  strip.background = element_blank(),
  strip.text = element_text(face = "bold", size = 10) 
) + ylab("Cell Density (cells/mm2)") + xlab("")
save_pdf(g, "patchwork_beeswarm.pdf", width=10, height=10)
```

```{r tmb-beeswarm}
rrt = rr[margin=="tumor_manual"]
g <- create_beeswarm_plot(rrt, x="tipMMR", y="tmb") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme(
  strip.background = element_blank(),
  strip.text = element_text(face = "bold", size = 10) 
) + ylab("TMB") + xlab("")
save_pdf(g, "tmb_beeswarm.pdf", width=2, height=2)
```

```{r cd3-beeswarm}
rrt = rr[margin=="tumor_manual"]
g <- create_beeswarm_plot(rrt, x="tipMMR", y="CD3_50r") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme(
  text = element_text(color = "black", size=9),
  axis.title = element_text(color = "black",size=9), # Axis titles
  axis.text = element_text(color = "black",size=9)) + 
  ylab("CD3") + xlab("")
save_pdf(g, "cd3_beeswarm.pdf", width=2, height=2)
```

```{r sma-beeswarm}
rrt = rr[margin=="tumor_manual"]
g <- create_beeswarm_plot(rrt, x="tipMMR", y="CD20_50r") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme(
  text = element_text(color = "black", size=9),
  axis.title = element_text(color = "black",size=9), # Axis titles
  axis.text = element_text(color = "black",size=9)) + 
  ylab("SMA") + xlab("") 
save_pdf(g, "sma_beeswarm.pdf", width=3, height=2)
```

```{r foxp3-beeswarm}
rrt = rr[margin=="tumor_manual"]
rm <- melt(rrt, id.vars=c("Slide_ID","tipMMR"), 
           measure.vars=c("PD_L1_50r","PD1_50r","FOXP3_50r"))

g <- create_beeswarm_plot(rm, x="tipMMR", y="value", withbox=TRUE) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) +
  theme(
    text = element_text(color = "black", size=9),
    axis.title = element_text(color = "black",size=9), # Axis titles
...
save_pdf(g, "treg_beeswarm.pdf", width=6, height=2)
```

```{r km-by-t-cell-count, inclue=FALSE}
# what margin are we operating on for this section?
rrt <- data.table::copy(rr[margin=="tumor_manual"])

# get the KM-plot data
d1 <- km_plot(rrt, parm="CD8_50r", parm.name="cd3", flip=FALSE, flip.labels=TRUE, return.data=TRUE)
d2 <- km_plot(rrt, parm="CD3_50r", parm.name="cd3", flip=FALSE, flip.labels=TRUE, return.data=TRUE)
d1[, strata := paste0(strata, "CD8")]
d2[, strata := paste0(strata, "CD3")]
dd <- rbind(d1, d2)

# plot the K-M curves
fill.vals <- c("1CD8"="#3182bd","0CD8"="#9ecae1", "1CD3"="#de2d26","0CD3"="#fc9272")
g <- ggplot(dd[time < 2400], aes(x = time/365, y = surv, color = strata)) +
    #geom_ribbon(aes(ymin = lower,ymax = upper, fill=strata),alpha = 0.15, linetype=0) +
    geom_step(aes(color=strata), linewidth=1) +
    geom_point(data = dd[censor == 1][time < 2400], aes(x=time/365,y=surv), shape="+", size=3, color="black") + 
    labs(x = "\nYears", y = ifelse(TRUE,"Progression-Free Survival","Overall Survival\n")) + theme_bw() + 
    #ggtitle(hr.lab) + 
    scale_color_manual(values=fill.vals) + 
    scale_x_continuous(breaks=s<-seq(0,6), labels=s) + 
    scale_fill_manual(values=fill.vals, guide="none") + 
...
save_pdf(g, "cd3_cd8_km.pdf", width=3, height=3)
```
...
# PanCK-CD3 double positive
rrt[, ratio := panckcd3count / cd3count]
g <- ggplot(rrt, aes(x = tipMMR, y = ratio, fill = tipMMR)) +
...
View(compare_groups(rrt, "ratio"))

# 0 flag cells
rrt[, ratio := `0_count` / allcount]
g <- ggplot(rrt, aes(x = tipMMR, y = ratio, fill = tipMMR)) +
  geom_boxplot(alpha = 1, width=0.8, outlier.shape=NA) + 
  geom_jitter(position = position_jitter(width = 0.2), size = 3, alpha = 1, shape=20) +  
  labs(y = "Uncounted cell count") +
  scale_fill_manual(values = pMMR_colors) +
  theme_jw() + theme(legend.position = "none")
View(compare_groups(rrt, "ratio"))
```

```{r cd163-margins, include=FALSE}
# subset to clean data
rrp <- data.table::copy(rr)

# calculate the CD163+ vs PanCK+ PD-L1 ratio
rrp[,pdl1cd163_ratio := pdl1cd163count / cd163count]
rrp[, pdl1panckcount_ratio := pdl1panckcount / panckcount]
rrp[, m0_tumor_ratio := pdl1cd163count / pdl1panckcount]
rrp[, m0_tumor_ratio_normalized := pdl1cd163count / pdl1panckcount * (panckcount / cd163count)]

to_remove_because_panck_no_work_tumor <- c("LSP10474","LSP10397","LSP10419","LSP10431","LSP10474",
                                           "LSP10507","LSP10551","LSP10683","LSP14418","LSP15288")
#rrp <- rrp[!Slide_ID %in% to_remove_because_panck_no_work_tumor]

# calculate margin / stroma difference
rr_ratio <- data.table(pdl1cd163_margin_ratio = rrp[margin=="margin", pdl1cd163count / allcount] / rrp[margin=="stroma_core", pdl1cd163count / allcount], Slide_ID = rrp[margin=="margin", Slide_ID], tipMMR=rrp[margin=="margin", tipMMR])

View(compare_groups(rr_ratio, "pdl1cd163_margin_ratio"))

# view the numerical differences
View(compare_groups(rrp[margin=="margin"], "m0_tumor_ratio"))
View(compare_groups(rrp[margin=="tumor"], "m0_tumor_ratio"))

# get the margin/core ratio
r0 <- rrp[margin=="margin",m0_tumor_ratio]/rrp[margin=="stroma_core",m0_tumor_ratio]
rrratio <- data.table::copy(rrp[margin=="margin"])
rrratio[, m0_tumor_ratio := r0]
rrratio[, margin := "margin_core_ratio"]
rrp <- rbind(rrp, rrratio)

# graph the differences
g <- create_beeswarm_plot(rrp, x_var="tipMMR", y_var = "m0_tumor_ratio", withbox=TRUE) + 
     facet_wrap(~margin, scales="free", nrow=3) +
     scale_fill_manual(values = pMMR_colors) + theme_minimal() + 
     #scale_y_log10(labels = scales::label_number()) +
     labs(x = "Margin Type", 
         y = "m0 / tumor PD-L1 ratio", 
         fill = "MMR Status", 
         color = "MMR Status") + theme_jw() + 
  theme(#legend.position = c(0.65, 0.3), 
        #legend.justification = c(-0.2, -0.3), # Coordinates relative to the plot area
        #legend.box.margin = margin(6, 6, 6, 6), # Adjust spacing around the legend box if necessary
        #legend.margin = margin(-10, -10, -10, -10),
  ...
  scale_x_discrete(labels=c("dMMR","td-pMMR","ti-pMMR"))
save_pdf(g, "m0_tumor_ratio.pdf", width=13, height=6)

# plot the 
g <- km_plot(rrp[tipMMR %in% c("tipMMR", "pMMR") & margin=="tumor"], parm="m0_tumor_ratio", parm.name="ratio", flip=FALSE, flip.labels=TRUE)

rrt <- data.table::copy(rrp[margin=="stroma_margin"])
setkey(rrt, m0_tumor_ratio)
clevels <- rrt[,Slide_ID]
rrt[, Slide_ID := factor(Slide_ID, levels=clevels)]
g <- ggplot(rrt, aes(x=Slide_ID, y=m0_tumor_ratio, fill=tipMMR)) + 
  geom_bar(stat="identity")+
  coord_flip() +
  scale_fill_manual(values=pMMR_colors) +
  theme_jw()
save_pdf(g, "m0_tumor_ratio_bar.pdf", width=5, height=8)
```
...
set.seed(42)
rrpmmr <- rrt[pMMR == "pMMR"]
hrdist <- sapply(seq(5000), function(cc) {
  if (cc %% 100 == 0)
    cat(cc,"\n")
  rrpmmr[, rand_label := "hi"]
  rrpmmr[sample(.N, 13), rand_label := "low"]
  hr <- as.numeric(km_plot(rrpmmr, parm="rand_label", parm.name="tipMMR",parm.labels=c(tipMMR="tipMMR",pMMR="pMMR"), flip=TRUE, flip.labels=TRUE, return.model=TRUE)$hr[1])
  return(hr)
})
...
g <- ggplot(data.table(hr=hrdist[hrdist < 10]), aes(x=hr)) + 
  geom_histogram(stat="bin", binwidth=0.05, fill="black", color="black") + 
  theme_jw() + 
  geom_vline(aes(xintercept=1), linewidth=1, color="red") +
 geom_vline(aes(xintercept=0.14), linewidth=1, color=pMMR_colors["tipMMR"]) + 
     theme(
        text = element_text(family = "Helvetica"),
        axis.line = element_line(colour = "black"),
        axis.text.x = element_text(color = "black"),
  ...
  scale_x_continuous(expand=c(0,0), breaks=c(0.5, 1, 2, 3, 4, 5)) + 
  scale_y_continuous(expand=c(0,0)) +
  coord_cartesian(xlim=c(0,5)) +
    xlab("Hazard Ratio") + 
  ylab("Frequency") 
save_pdf(g, "tipmmr_random_permutation.pdf", width=5, height=3)
...
rr_complete <- as.data.table(tidyr::complete(rrt, Location, tipMMR, fill = list(n = 0)))
...
ix <- redcap[, pMMR=="pMMR"]
redcap[, PFSC := ifelse(redcap$PFSCensor==0,1,0)]
survival_object <- Surv(redcap$PFSDays[ix],redcap$PFSC[ix])
cox_model <- coxph(survival_object ~ redcap$Gender[ix] + redcap$mucinous[ix] + redcap$Grade[ix] + redcap$LVI[ix] + redcap$PNI[ix] + redcap$stage_num[ix] + redcap$APC[ix] + redcap$KRAS[ix] + redcap$TP53[ix] +
                     redcap$BRAF[ix] + redcap$PIK3CA[ix] + redcap$tmb[ix] + redcap$TIL_num[ix])

#Cox-Snell R2
coxsnellR2(cox_model)

# Initialize results
covariates <- c("Gender", "Age", "stage_num","mucinous", "Grade", "LVI", "PNI", "TIL_num","tmb","APC", "KRAS", "TP53", "BRAF", "PIK3CA")
results <- data.table(Covariates = character(), R2 = numeric(), R2Lower = numeric(), R2Upper = numeric())

# Loop through covariates
for (i in seq_along(covariates)) {
    current_covariates <- covariates[1:i]
    covariate_names <- paste(covariates[i])
    cox_model <- coxph(survival_object ~ ., data = redcap[ix, ..current_covariates])
    r2 <- coxsnellR2(cox_model)
    
    # Bootstrap for error estimates
    boot_results <- boot(data = rr[margin=="tumor_core"][ix], statistic = bootstrapR2, R = 1000, covariates = current_covariates)
    lower <- quantile(boot_results$t, probs = 0.025)
    upper <- quantile(boot_results$t, probs = 0.975)

    # Store results
    results <- rbind(results, data.frame(cov = covariate_names, R2 = r2, R2Lower = lower, R2Upper = upper))
}

# Plot results
library(ggplot2)
results[, Covariates := c("APC"="APC", "TP53"="TP53",
                       "Age"="Age","PIK3CA"="PIK3CA",
                       "BRAF"="BRAF",
                       "TIL_num"="TIL",
                       "mucinous"="Mucinous", "KRAS"="KRAS",
                       "Gender"="Gender","tmb"="TMB",
                       "Grade"="Grade","PNI"="PNI","LVI"="LVI",
                       "stage_num"="TNM Stage","CD8_50r"="CD8+ density")[cov]]
setkey(results, R2)
results$Covariates <- factor(results$Covariates, levels = unique(results$Covariates))
g <- ggplot(results, aes(x = Covariates, y = R2)) +
    geom_point() +
    geom_errorbar(aes(ymin = R2Lower, ymax = R2Upper), width = 0.1) +
    xlab("Covariates") +
    ylab("Cox-Snell R-squared") +
    ggtitle("Cox-Snell R-squared vs. Covariates") +
    theme_jw() +     theme(axis.text.x = element_text(angle = 90, hjust = 1))
save_pdf(g, "cox_snell_r2_noimaging.pdf", width=4, height=3)
...
ix <- redcap[, pMMR=="pMMR"]
redcap[, PFSC := ifelse(redcap$PFSCensor==0,1,0)]
survival_object <- Surv(rrt$PFSDays[ix],rrt$PFSC[ix])
cox_model <- coxph(survival_object ~ redcap$Gender[ix] + redcap$mucinous[ix] + redcap$Grade[ix] + redcap$LVI[ix] + redcap$PNI[ix] + redcap$stage_num[ix] + redcap$APC[ix] + redcap$KRAS[ix] + redcap$TP53[ix] +
                     redcap$BRAF[ix] + redcap$PIK3CA[ix] + redcap$tmb[ix] + redcap$TIL_num[ix] + rrt$CD3_50r[ix])

#Cox-Snell R2
coxsnellR2(cox_model)

# Initialize results
covariates <- c("Gender", "Age", "stage_num","mucinous", "Grade", "LVI", "PNI", "TIL_num","tmb","APC", "KRAS", "TP53", "BRAF", "PIK3CA", "CD3_50r")
results <- data.table(Covariates = character(), R2 = numeric(), R2Lower = numeric(), R2Upper = numeric())

# Loop through covariates
for (i in seq_along(covariates)) {
    current_covariates <- covariates[i]
    covariate_names <- paste(covariates[i])
    cox_model <- coxph(survival_object ~ ., data = rrt[ix, ..current_covariates])
    r2 <- coxsnellR2(cox_model)
    
    # Bootstrap for error estimates
    boot_results <- boot(data = rrt[ix], statistic = bootstrapR2, R = 1000, covariates = current_covariates)
    lower <- quantile(boot_results$t, probs = 0.025)
    upper <- quantile(boot_results$t, probs = 0.975)

    # Store results
    results <- rbind(results, data.frame(Covariates = covariate_names, R2 = r2, R2Lower = lower, R2Upper = upper))
}

# Plot results
library(ggplot2)
replacements <- c("Mucinous" = "mucinous", "Sex (M)" = "Gender",
                  "TIL"="TIL_num","TNM stage"="stage_num",
                  "Intra-tumoral CD3"="CD3_50r","TMB"="tmb")
# Loop over the replacements and update the 'variable' column
for (old in names(replacements)) {
  results[Covariates == replacements[old], Covariates := old]
}
setkey(results, R2)
results$Covariates <- factor(results$Covariates, levels = unique(results$Covariates))
g <- ggplot(results, aes(x = Covariates, y = R2)) +
...
save_pdf(g, "cox_snell_r2_univariate.pdf", width=4, height=3)
...
```{r cox-ph-full}
rrt=rr[margin=="tumor_manual"]
ix <- rep(TRUE, nrow(rrt))
ix <- rrt[, pMMR=="pMMR"]
rrt[, PFSC := ifelse(rrt$PFSCensor==0,1,0)]
survival_object <- Surv(rrt$PFSDays[ix],rrt$PFSC[ix])
cox_model <- coxph(survival_object ~ rrt$Gender[ix] + rrt$mucinous[ix] + rrt$Grade[ix] + rrt$LVI[ix] + rrt$PNI[ix] + rrt$stage_num[ix] + rrt$APC[ix] + rrt$KRAS[ix] + rrt$TP53[ix] +
                     rrt$BRAF[ix] + rrt$PIK3CA[ix] + rrt$tmb[ix] + rrt$TIL_num[ix] + rrt$CD8_50l[ix])

#Cox-Snell R2
coxsnellR2(cox_model)

# Initialize results
covariates <- c("Gender", "Age", "stage_num","mucinous", "Grade", "LVI", "PNI", "TIL_num","tmb","APC", "KRAS", "TP53", "BRAF", "PIK3CA", "CD8_50l")
results <- data.table(Covariates = character(), R2 = numeric(), R2Lower = numeric(), R2Upper = numeric())
cov_names <- c("APC"="APC", "TP53"="TP53",
                       "Age"="Age","PIK3CA"="PIK3CA",
                       "BRAF"="BRAF",
                       "TIL_num"="TIL",
                       "mucinous"="Mucinous", "KRAS"="KRAS",
                       "Gender"="Gender","tmb"="TMB",
                       "Grade"="Grade","PNI"="PNI","LVI"="LVI",
                       "stage_num"="TNM Stage","CD8_50l"="CD8+ density")
stopifnot(all(covariates %in% names(cov_names)))
...
for (i in seq_along(covariates)) {
    ...
    boot_results <- boot(data = rr[margin=="tumor_core"][ix], statistic = bootstrapR2, R = 1000, covariates = current_covariates)
    lower <- quantile(boot_results$t, probs = 0.025)
    r2 = quantile(boot_results$t, probs=0.50)
    upper <- quantile(boot_results$t, probs = 0.975)
...
# Loop through covariates UNIVARIATE
results <- data.table(Covariates = character(), R2 = numeric(), R2Lower = numeric(), R2Upper = numeric())
for (i in seq_along(covariates)) {
    current_covariates <- covariates[i]
    covariate_names <- cov_names[covariates[i]]
    cox_model <- coxph(survival_object ~ ., data = rrt[ix, ..current_covariates])
    r2 <- coxsnellR2(cox_model)
...
library(ggplot2)
setkey(results, R2)
results$Covariates <- factor(results$Covariates, levels = unique(results$Covariates)[order(results$R2)])
g <- ggplot(results, aes(x = Covariates, y = R2)) +
    geom_point() +
    geom_errorbar(aes(ymin = R2Lower, ymax = R2Upper), width = 0.1) +
    xlab("Covariates") +
...
save_pdf(g, "cox_snell_r2_full.pdf", width=4, height=3)
...
rr_long <- melt(rrt, id.vars = "tipMMR", measure.vars = c("CD20_200r", "PD1_200r","PD_L1_200r"), variable.name = "Metric", value.name = "Value")
wilcox.test(rr[pMMR=="tipMMR", CD20_200r], rr[pMMR=="dMMR", CD20_200r]) # p = 0.06399
wilcox.test(rr[tipMMR=="tipMMR", PD1_200r], rr[tipMMR=="dMMR", PD1_200r]) # p = 0.09163
...
rrm <- melt(rr, id.vars = "tipMMR", measure.vars = c("BRAF", "APC", "CTNNB1", "KRAS", "TP53","PIK3CA"), variable.name="Gene")
...
rrt[, ab := CD163pPDL1_200r / PanCKpPDL1_200r]
g <- ggplot(rrt, aes(x = tipMMR, y = ab, fill = tipMMR)) +
  geom_boxplot(alpha = 0.5, width=0.8, outlier.shape=NA) +
  geom_jitter(position = position_jitter(width = 0.2), size = 1, alpha = 0.7, shape=20) +
  ...
  scale_fill_manual(values = pMMR_colors) +
  theme_jw()
wilcox.test(rrt[tipMMR=="tipMMR", ab], rrt[tipMMR=="dMMR", ab])
save_pdf(g, "m0_tumor_pdl1_ratio.pdf", width=3, height=3)

rr_long <- melt(rrt, id.vars = "tipMMR", measure.vars = c("PD1_200r", "PD_L1_200r", "CD45RO_200r", "PanCKpPDL1_200r","CD163pPDL1_200r", "ab"), variable.name = "Metric", value.name = "Value")
rr_long[, cells_per_mm2 := Value * 1e6 / (1000^2 * 0.325^2)]
...
#1 tumor
#2 margin
dt <- fread(cmd=paste("cysift cut -x Region","~/Sorger/projects/orion/orion_1_74/LSP10507.ptrd.cys", "- | cysift view - -R"))
dt <- dt[x > 400 & x < 500 & y > 5000 & y < 5500]
dt[, Region := as.factor(Region)]
dt[, panck := ifelse(bitAnd(pflag, 131072) > 0,"panck","no")]
dt[, tumor := ifelse(bitAnd(cflag, 1), "tumor","stroma")]
dt[, tumor_region := paste0(tumor,Region)]
dt[x==453 & y == 5421, tumor_region := "special"]
g <- ggplot(dt, aes(x=x,y=y, color=tumor_region, shape=panck)) + geom_point() + 
  scale_color_manual(values=c("tumor1"="red","tumor2"="darkred", "stroma0"="gray","stroma1"="black","stroma2"="cyan","tumor0"="purple","special"="pink" ))+ 
  coord_fixed()
```
