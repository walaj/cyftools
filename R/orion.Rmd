...
if (FALSE) {
  ...
  dtt <- lapply(files, function(f) {
    ...
    cat("...cell count\n")
    drr = fread(cmd=paste("cysift cellcount", tmpfile," - -a 133120 -a 10240 -a 36864 -a 32832 | cysift view -"))
  ...
  saveRDS(dtr, "~/Sorger/cache/stroma_N5_cut.rds", compress=FALSE)
} else {
  ...
  dtr <- rbindlist(lapply(c("~/Sorger/cache/stroma_A4N1_cut.rds",
                            "~/Sorger/cache/core_A1N4_cut.rds",
                            "~/Sorger/cache/stroma_N5_cut.rds",
                            "~/Sorger/cache/tumor_A5_cut.rds"), readRDS))
  rr <- merge(data.table::copy(redcap), dtr, by = "Slide_ID", all.x = TRUE)
...
```{r imaging-km}
rr <- merge(data.table::copy(redcap), dtr[margin=="core1"], by = "Slide_ID", all.x = TRUE)
ix <- rep(TRUE, nrow(rr))
survival_object <- Surv(rr$PFSDays[ix], ifelse(rr$PFSCensor[ix]==0,1,0))
...
all_hr <- lapply(all_200r, function(x) {
  ...
  cox_model <- coxph(survival_object ~ rr[[x]][ix] >= median_cut)
cat(x, "\n")
