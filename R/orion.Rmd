...
if (FALSE) {
  ...
  dtt <- lapply(files, function(f) {
    cat(basename(f), "\n")
    dt <- fread(cmd=paste("cysift filter",f,"/tmp/tmp.cys && cysift mean /tmp/tmp.cys - | cysift view -"))
  ...
  saveRDS(dtr, "~/Sorger/cache/stroma_n5.rds", compress=FALSE)
} else {
...
```{r margins}
rr_long <- melt(rr, id.vars = c("tipMMR","margin"), measure.vars = c("SMA_200g", "CD68_200g","CD163_200g"),
