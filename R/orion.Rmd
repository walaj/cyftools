...
if (FALSE) {
  files <- list.files(path = "~/Sorger/projects/orion/orion_1_74/margin_nov/", full.names = TRUE)
  ...
  dtt <- lapply(files, function(f) {
    ...
    ccmd=paste("cysift scramble",f, "- | cysift filter -", fv, tmpfile)
    #ccmd=paste("cysift filter",f, fv,tmpfile)
    ...
    drr = fread(cmd=paste("cysift cellcount", tmpfile," - -a 133120 -a 10240 -a 36864 -a 32832 -a 0 | cysift view - -R"))
    dt[, cd31count := drr$CD31_count]
    dt[, cd45count := drr$CD45_count]
    dt[, cd68count := drr$CD68_count]
    dt[ ,cd4count. := drr$CD4_count]
    dt[, foxp3count:= drr$FOXP3_count]
    dt[, cd8count  := drr$CD8a_count]
    dt[, cd45rocount  := drr$CD45RO_count]
    dt[, cd20count := drr$CD20_count]
    dt[, pdl1count := drr$`PD-L1_count`]
    dt[, cd3count := drr$CD3e_count]
    dt[, cd163count := drr$CD163_count]
    dt[, pd1count := drr$`PD-1_count`]
    dt[, panckcount := drr$`Pan-CK_count`]
    dt[, smacount := drr$SMA_count]
    dt[ , pdl1panckcount := drr$`133120_count`]
    dt[ , pdl1cd163count := drr$`10240_count`]
    dt[ , cd3pd1count    := drr$`36864_count`]
    dt[ , cd4pd1count    := drr$`32832_count`]
  ...
  saveRDS(dtr, "~/Sorger/cache/scramble_margin_Nov3.rds", compress=FALSE)
} else {
  ...
  dtr <- rbindlist(lapply(c("~/Sorger/cache/stroma_core_Nov3.rds",
                            "~/Sorger/cache/all_margin_Nov3.rds",
                            "~/Sorger/cache/tumor_core_Nov3.rds",
                            "~/Sorger/cache/scramble_margin_Nov3.rds",
                            "~/Sorger/cache/stroma_margin_Nov3.rds"), readRDS))
  rr <- merge(data.table::copy(redcap), dtr, by = "Slide_ID", all.x = TRUE)
...
# Initialize results
covariates <- c("Gender", "Age", "stage_num","mucinous", "Grade", "LVI", "PNI", "TIL_num","tmb","APC", "KRAS", "p53", "BRAF", "PIK3CA")
...
for (i in seq_along(covariates)) {
    ...
    boot_results <- boot(data = redcap[ix], statistic = bootstrapR2, R = 1000, covariates = current_covariates)
...
dt <- fread(cmd=paste("cysift view","~/Sorger/projects/orion/orion_1_74/chain/LSP10353.ptrd.cys", "-R"))
dt <- dt[x < 5000 & y < 5000]
dt[, Region := as.factor(Region)]
g <- ggplot(dt, aes(x=x,y=y)) + geom_point(aes(color=Region))
```
